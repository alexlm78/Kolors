...
/**
 * Comprehensive test suite that validates all major functionality This serves
 * as a validation that the end-to-end tests are properly implemented
 */
@SpringBootTest
...
class ComprehensiveTestSuite {
    ...
    void shouldValidateTestInfrastructure() {
        ...
        ColorCombination updated = colorCombinationService.addColorToCombination(created.getId(), newColor);
        assertThat(updated.getColors()).hasSize(3);

        // Test 3: Color removal with reordering
        ColorCombination afterRemoval = colorCombinationService.removeColorFromCombination(created.getId(), 2);
        assertThat(afterRemoval.getColors()).hasSize(2);

        List<ColorInCombination> colors = colorInCombinationRepository
                .findByCombinationIdOrderByPosition(created.getId());
        assertThat(colors.get(0).getPosition()).isEqualTo(1);
        assertThat(colors.get(1).getPosition()).isEqualTo(2);

        // Test 4: Search functionality
        List<ColorCombination> searchResults = colorCombinationService.searchCombinations("Test", null);
        ...
        assertThat(colorInCombinationRepository.findByCombinationIdOrderByPosition(created.getId())).isEmpty();
    }
    ...
    void shouldValidatePerformanceWithMultipleCombinations() {
        ...
        List<ColorCombination> searchResults = colorCombinationService.searchCombinations("Performance", null);
        assertThat(searchResults).hasSize(50);

        // Test color count filtering
        List<ColorCombination> singleColorCombinations = colorCombinationService.searchCombinations(null, 1);
