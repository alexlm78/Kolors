...
/**
 * Performance tests for critical operations Tests system behavior under load and with large
 * datasets
 */
@SpringBootTest
...
class PerformanceIntegrationTest {
    ...
    class BulkOperationsPerformanceTests {
        ...
        void shouldCreateLargeNumberOfCombinationsWithinThreshold() {
            ...
            long expectedColors =
                    IntStream.range(0, LARGE_DATASET_SIZE).map(i -> (i % 4) + 1).sum();
        ...
        void shouldHandleBulkUpdatesWithinThreshold() {
            ...
            for (ColorCombination combination : combinations) {
                ColorCombinationForm updateForm =
                        createTestForm("Updated " + combination.getId(), 3);
    ...
    class SearchPerformanceTests {
        ...
        void setupLargeDataset() {
            ...
            for (int i = 0; i < LARGE_DATASET_SIZE; i++) {
                ...
                for (int j = 0; j < colorCount; j++) {
                    String hex = colorBases[j % colorBases.length] + String.format("%04X", i + j);
        ...
        void shouldSearchByNameWithinThreshold() {
            ...
            List<ColorCombination> results =
                    colorCombinationService.searchCombinations("Ocean", null);
        ...
        void shouldHandleComplexSearchQueriesWithinThreshold() {
            ...
            List<ColorCombination> nameResults =
                    colorCombinationService.searchCombinations("Sunset", null);
            List<ColorCombination> countResults =
                    colorCombinationService.searchCombinations(null, 2);
            List<ColorCombination> combinedResults =
                    colorCombinationService.searchCombinations("Forest", 4);
    ...
    class DynamicColorManagementPerformanceTests {
        ...
        @Test
        @DisplayName(
                "Should handle rapid color removals with reordering within performance threshold")
    ...
    class ConcurrentOperationsPerformanceTests {
        ...
        void shouldHandleConcurrentSearches() throws Exception {
            ...
            for (int i = 0; i < 100; i++) {
                ...
                CompletableFuture<List<ColorCombination>> future =
                        CompletableFuture.supplyAsync(
                                () -> {
                                    if (searchId % 3 == 0) {
                                        return colorCombinationService.searchCombinations(
                                                "Search", null);
    ...
    private ColorCombinationForm createTestForm(String name, int colorCount) {
        ...
        for (int i = 0; i < colorCount; i++) {
            ...
            String hex =
                    String.format("%02X%02X%02X", (i * 50) % 256, (i * 100) % 256, (i * 150) % 256);
            form.addColor(hex);
        }
        return form;
    }
