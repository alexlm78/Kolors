...
class PerformanceIntegrationTest {
  ...
  class BulkOperationsPerformanceTests {
    ...
    void shouldCreateLargeNumberOfCombinationsWithinThreshold() {
      ...
      long expectedColors = IntStream.range(0, LARGE_DATASET_SIZE).map(i -> (i % 4) + 1).sum();
                ...
                () -> {
                  for (int i = 0; i < 10; i++) {
                    ColorCombinationForm form =
                        createTestForm("Concurrent " + threadId + "-" + i, (i % 3) + 1);
                    colorCombinationService.createCombination(form);
                  }
                },
                executor);
  ...
  private ColorCombinationForm createTestForm(String name, int colorCount) {
    ...
    for (int i = 0; i < colorCount; i++) {
      ...
      String hex = String.format("%02X%02X%02X", (i * 50) % 256, (i * 100) % 256, (i * 150) % 256);
      form.addColor(hex);
    }
    return form;
  }
