...
/**
 * Tests de integración para ColorCombinationController Enfocados en la lógica del controlador sin
 * requerir templates
 */
@SpringBootTest
@AutoConfigureWebMvc
...
class ColorCombinationControllerIntegrationTest {
   ...
   class IndexPageTests {
      ...
      void shouldRespondToIndexPage() throws Exception {
         ...
         // When & Then
         mockMvc.perform(get("/combinations/")).andExpect(status().isOk())
      ...
      void shouldHandleSearchParameters() throws Exception {
         ...
         Page<ColorCombination> page = new PageImpl<>(testCombinations);
         when(colorCombinationService.searchWithFilters(eq(searchTerm), isNull(), isNull(),
                  isNull(), any(Pageable.class))).thenReturn(page);

         // When & Then
         mockMvc.perform(get("/combinations/").param("search", searchTerm))
                  .andExpect(status().isOk()).andExpect(model().attribute("search", searchTerm));

         verify(colorCombinationService).searchWithFilters(eq(searchTerm), isNull(), isNull(),
      ...
      void shouldHandleHexValueSearch() throws Exception {
         ...
         mockMvc.perform(get("/combinations/").param("hexValue", hexValue))
                  .andExpect(status().isOk()).andExpect(model().attribute("hexValue", hexValue));

         verify(colorCombinationService).searchWithFilters(isNull(), isNull(), isNull(),
                  eq(hexValue), any(Pageable.class));
      }
   }
   ...
   class CreateCombinationTests {
      ...
      void shouldCreateValidCombinationSuccessfully() throws Exception {
         ...
         // When & Then
         mockMvc.perform(post("/combinations/create").param("name", "Ocean Breeze")
                  .param("colorCount", "2").param("colors[0].hexValue", "0077BE")
                  .param("colors[0].position", "1").param("colors[1].hexValue", "87CEEB")
                  .param("colors[1].position", "2")).andExpect(status().is3xxRedirection())
      ...
      void shouldHandleServiceValidationErrors() throws Exception {
         ...
         mockMvc.perform(post("/combinations/create").param("name", "Test Combination")
                  .param("colorCount", "2").param("colors[0].hexValue", "0077BE")
                  .param("colors[0].position", "1").param("colors[1].hexValue", "87CEEB")
                  .param("colors[1].position", "2")).andExpect(status().isOk())
   ...
   class EditCombinationTests {
      ...
      void shouldDisplayEditForm() throws Exception {
         ...
         // When & Then
         mockMvc.perform(get("/combinations/1/edit")).andExpect(status().isOk())
                  .andExpect(model().attributeExists("combinationForm"))
      ...
      void shouldUpdateCombinationSuccessfully() throws Exception {
         ...
         // When & Then
         mockMvc.perform(post("/combinations/1/update").param("name", "Updated Ocean Breeze")
                  .param("colorCount", "2").param("colors[0].hexValue", "0077BE")
                  .param("colors[0].position", "1").param("colors[1].hexValue", "87CEEB")
                  .param("colors[1].position", "2")).andExpect(status().is3xxRedirection())
                  .andExpect(redirectedUrl("/combinations/"))
   ...
   class DeleteCombinationTests {
      ...
      void shouldDisplayDeleteConfirmation() throws Exception {
         ...
         // When & Then
         mockMvc.perform(get("/combinations/1/confirm-delete")).andExpect(status().isOk())
                  .andExpect(model().attributeExists("combination"));
      ...
      void shouldDeleteCombinationSuccessfully() throws Exception {
         ...
         // When & Then
         mockMvc.perform(post("/combinations/1/delete")).andExpect(status().is3xxRedirection())
                  .andExpect(redirectedUrl("/combinations/"))
                  .andExpect(flash().attributeExists("success"));
   ...
   class ViewCombinationTests {
      ...
      void shouldDisplayCombinationDetails() throws Exception {
         ...
         // When & Then
         mockMvc.perform(get("/combinations/1")).andExpect(status().isOk())
                  .andExpect(model().attributeExists("combination"));
   ...
   class AjaxSearchTests {
      ...
      void shouldHandleAjaxSearchByTerm() throws Exception {
         ...
         verify(colorCombinationService).searchWithFilters(eq("Ocean"), isNull(), isNull(),
                  isNull());
      }
   ...
   class ErrorHandlingTests {
      ...
      void shouldHandleErrorsInDelete() throws Exception {
         // Given
         when(colorCombinationService.getById(1L)).thenReturn(testCombination);
         doThrow(new RuntimeException("Delete error")).when(colorCombinationService)
   ...
   class DynamicColorManagementTests {
      ...
      void shouldAddColorToCombinationSuccessfully() throws Exception {
         ...
         // When & Then
         mockMvc.perform(post("/combinations/1/add-color").param("hexValue", "FFFF00")
                  .param("position", "4")).andExpect(status().is3xxRedirection())
      ...
      void shouldHandleValidationErrorsWhenAddingColor() throws Exception {
         ...
         mockMvc.perform(post("/combinations/1/add-color").param("hexValue", "INVALID")
                  .param("position", "4")).andExpect(status().is3xxRedirection())
                  .andExpect(redirectedUrl("/combinations/1/edit"))
                  .andExpect(flash().attributeExists("error"));
      }
      ...
      void shouldHandleCombinationNotFoundWhenAddingColor() throws Exception {
         ...
         // When & Then
         mockMvc.perform(post("/combinations/999/add-color").param("hexValue", "FFFF00")
                  .param("position", "4")).andExpect(status().is3xxRedirection())
                  .andExpect(redirectedUrl("/combinations/"))
                  .andExpect(flash().attributeExists("error"));
      }
      ...
      void shouldHandleValidationErrorWhenRemovingLastColor() throws Exception {
         // Given
         when(colorCombinationService.removeColorFromCombination(1L, 1)).thenThrow(
                  new ColorCombinationValidationException("Cannot remove the last color"));
