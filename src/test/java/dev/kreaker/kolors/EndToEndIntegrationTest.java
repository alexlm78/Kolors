...
class EndToEndIntegrationTest {
   ...
   class CompleteUserJourneyTests {
      ...
      void shouldCompleteFullCrudLifecycle() throws Exception {
         ...
         mockMvc.perform(post("/combinations/create").param("name", "Sunset Palette")
                  .param("colorCount", "3").param("colors[0].hexValue", "FF6B35")
                  .param("colors[0].position", "1").param("colors[1].hexValue", "F7931E")
                  .param("colors[1].position", "2").param("colors[2].hexValue", "FFD23F")
                  .param("colors[2].position", "3")).andExpect(status().is3xxRedirection())
         ...
         mockMvc.perform(get("/combinations/" + combinationId)).andExpect(status().isOk())
                  .andExpect(view().name("combinations/view"))
                  .andExpect(model().attributeExists("combination"));

         // Step 3: Edit the combination
         mockMvc.perform(get("/combinations/" + combinationId + "/edit")).andExpect(status().isOk())
         ...
         mockMvc.perform(post("/combinations/" + combinationId + "/update")
                  .param("name", "Updated Sunset Palette").param("colorCount", "3")
                  .param("colors[0].hexValue", "FF6B35").param("colors[0].position", "1")
                  .param("colors[1].hexValue", "F7931E").param("colors[1].position", "2")
                  .param("colors[2].hexValue", "FFD23F").param("colors[2].position", "3"))
                  .andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/combinations/"))
                  .andExpect(flash().attributeExists("success"));
         ...
         mockMvc.perform(get("/combinations/" + combinationId + "/confirm-delete"))
                  .andExpect(status().isOk()).andExpect(view().name("combinations/confirm-delete"))
                  .andExpect(model().attributeExists("combination"));

         // Step 6: Delete the combination
         mockMvc.perform(post("/combinations/" + combinationId + "/delete"))
                  .andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/combinations/"))
                  .andExpect(flash().attributeExists("success"));
   ...
   class DynamicColorManagementE2ETests {
      ...
      void shouldHandleCompleteDynamicColorWorkflow() throws Exception {
         ...
         mockMvc.perform(post("/combinations/create").param("name", "Growing Palette")
                  .param("colorCount", "1").param("colors[0].hexValue", "FF0000")
                  .param("colors[0].position", "1")).andExpect(status().is3xxRedirection())
                  .andExpect(redirectedUrl("/combinations/"));
         ...
         mockMvc.perform(post("/combinations/" + combinationId + "/add-color")
                  .param("hexValue", "00FF00").param("position", "2"))
         ...
         mockMvc.perform(post("/combinations/" + combinationId + "/add-color")
                  .param("hexValue", "0000FF").param("position", "3"))
                  .andExpect(status().is3xxRedirection())
                  .andExpect(flash().attributeExists("success"));
      ...
      void shouldHandleAddingMultipleColorsSequentially() throws Exception {
         ...
         mockMvc.perform(post("/combinations/create").param("name", "Sequential Palette")
                  .param("colorCount", "1").param("colors[0].hexValue", "111111")
                  .param("colors[0].position", "1")).andExpect(status().is3xxRedirection());
   ...
   class SearchAndFilterE2ETests {
      ...
      void shouldHandleCompleteSearchAndFilterWorkflow() throws Exception {
         ...
         // Test search by name
         mockMvc.perform(get("/combinations/").param("search", "Ocean")).andExpect(status().isOk())
                  .andExpect(model().attributeExists("combinations"))
                  .andExpect(model().attribute("search", "Ocean"));

         // Test search by color count
         mockMvc.perform(get("/combinations/").param("colorCount", "3")).andExpect(status().isOk())
                  .andExpect(model().attributeExists("combinations"))
                  .andExpect(model().attribute("colorCount", 3));

         // Test search by hex value
         mockMvc.perform(get("/combinations/").param("hexValue", "0077BE"))
                  .andExpect(status().isOk()).andExpect(model().attributeExists("combinations"))
                  .andExpect(model().attribute("hexValue", "0077BE"));

         // Test AJAX search endpoint
         mockMvc.perform(get("/combinations/search").param("term", "Forest"))
                  .andExpect(status().isOk()).andExpect(model().attributeExists("combinations"));

         // Test combined search criteria
         mockMvc.perform(get("/combinations/").param("search", "Ocean").param("colorCount", "2"))
                  .andExpect(status().isOk()).andExpect(model().attributeExists("combinations"));
      }
      ...
      private void createTestCombination(String name, String... hexValues) throws Exception {
         ...
         mockMvc.perform(post("/combinations/create?" + params))
                  .andExpect(status().is3xxRedirection());
      }
   }
   ...
   class ErrorHandlingE2ETests {
      ...
      void shouldHandleValidationErrorsThroughoutWorkflow() throws Exception {
         ...
         mockMvc.perform(post("/combinations/create").param("name", "ab") // Too short
                  .param("colorCount", "1").param("colors[0].hexValue", "INVALID") // Invalid hex
                  .param("colors[0].position", "1")).andExpect(status().isOk())
                  .andExpect(model().attributeExists("error"));

         // Test creation with empty name
         mockMvc.perform(post("/combinations/create").param("name", "").param("colorCount", "1")
                  .param("colors[0].hexValue", "FF0000").param("colors[0].position", "1"))
         ...
         mockMvc.perform(get("/combinations/999")).andExpect(status().is3xxRedirection())
                  .andExpect(redirectedUrl("/combinations/"))
                  .andExpect(flash().attributeExists("error"));

         // Test editing non-existent combination
         mockMvc.perform(get("/combinations/999/edit")).andExpect(status().is3xxRedirection())
                  .andExpect(redirectedUrl("/combinations/"))
                  .andExpect(flash().attributeExists("error"));

         // Test deleting non-existent combination
         mockMvc.perform(post("/combinations/999/delete")).andExpect(status().is3xxRedirection())
                  .andExpect(redirectedUrl("/combinations/"))
                  .andExpect(flash().attributeExists("error"));
      }
      ...
      void shouldHandleDynamicColorManagementErrors() throws Exception {
         ...
         mockMvc.perform(post("/combinations/create").param("name", "Test Combination")
                  .param("colorCount", "1").param("colors[0].hexValue", "FF0000")
                  .param("colors[0].position", "1")).andExpect(status().is3xxRedirection());
         ...
         mockMvc.perform(post("/combinations/" + combinationId + "/add-color")
                  .param("hexValue", "INVALID").param("position", "2"))
                  .andExpect(status().is3xxRedirection())
                  .andExpect(redirectedUrl("/combinations/" + combinationId + "/edit"))
                  .andExpect(flash().attributeExists("error"));

         // Test adding color to non-existent combination
         mockMvc.perform(post("/combinations/999/add-color").param("hexValue", "00FF00")
                  .param("position", "2")).andExpect(status().is3xxRedirection())
                  .andExpect(redirectedUrl("/combinations/"))
                  .andExpect(flash().attributeExists("error"));
         ...
         mockMvc.perform(post("/combinations/" + combinationId + "/remove-color/999"))
                  .andExpect(status().is3xxRedirection())
                  .andExpect(redirectedUrl("/combinations/" + combinationId + "/edit"))
                  .andExpect(flash().attributeExists("error"));
      }
   ...
   class DataConsistencyE2ETests {
      ...
      void shouldMaintainDataConsistencyDuringComplexOperations() throws Exception {
         ...
         mockMvc.perform(post("/combinations/create").param("name", "Consistency Test")
                  .param("colorCount", "4").param("colors[0].hexValue", "FF0000")
                  .param("colors[0].position", "1").param("colors[1].hexValue", "00FF00")
                  .param("colors[1].position", "2").param("colors[2].hexValue", "0000FF")
                  .param("colors[2].position", "3").param("colors[3].hexValue", "FFFF00")
                  .param("colors[3].position", "4")).andExpect(status().is3xxRedirection());
         ...
         // Add new color
         mockMvc.perform(post("/combinations/" + combinationId + "/add-color")
                  .param("hexValue", "FF00FF").param("position", "4"))
                  .andExpect(status().is3xxRedirection())
                  .andExpect(flash().attributeExists("success"));
         ...
         // Update combination
         mockMvc.perform(post("/combinations/" + combinationId + "/update")
                  .param("name", "Updated Consistency Test").param("colorCount", "4")
                  .param("colors[0].hexValue", "FF0000").param("colors[0].position", "1")
                  .param("colors[1].hexValue", "0000FF").param("colors[1].position", "2")
                  .param("colors[2].hexValue", "FFFF00").param("colors[2].position", "3")
                  .param("colors[3].hexValue", "FF00FF").param("colors[3].position", "4"))
                  .andExpect(status().is3xxRedirection())
                  .andExpect(flash().attributeExists("success"));
