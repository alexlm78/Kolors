...
class ColorCombinationRestControllerTest {
   ...
   void shouldAddColorViaAjaxSuccessfully() throws Exception {
      ...
      // When & Then
      mockMvc.perform(post("/api/combinations/1/colors").with(csrf())
               .contentType(MediaType.APPLICATION_JSON)
               .content(objectMapper.writeValueAsString(testColorForm))).andExpect(status().isOk())
   ...
   void shouldHandleCombinationNotFoundWhenAddingColorViaAjax() throws Exception {
      ...
      // When & Then
      mockMvc.perform(post("/api/combinations/999/colors").with(csrf())
               .contentType(MediaType.APPLICATION_JSON)
   ...
   void shouldValidateColorFormatViaAjax() throws Exception {
      ...
      // When & Then
      mockMvc.perform(post("/api/combinations/validate-color").with(csrf())
               .contentType(MediaType.APPLICATION_JSON)
               .content(objectMapper.writeValueAsString(validColorForm))).andExpect(status().isOk())
               .andExpect(content().contentType(MediaType.APPLICATION_JSON))
   ...
   void shouldGetCombinationDetailsViaAjax() throws Exception {
      ...
      // When & Then
      mockMvc.perform(post("/api/combinations/1").with(csrf())).andExpect(status().isOk())
               .andExpect(content().contentType(MediaType.APPLICATION_JSON))
               .andExpect(jsonPath("$.success").value(true))
               .andExpect(jsonPath("$.combination.id").value(1))
               .andExpect(jsonPath("$.combination.name").value("Test Combination"))
   ...
   void shouldHandleCombinationNotFoundWhenGettingDetailsViaAjax() throws Exception {
      ...
      // When & Then
      mockMvc.perform(post("/api/combinations/999").with(csrf())).andExpect(status().isNotFound());
   }
}
