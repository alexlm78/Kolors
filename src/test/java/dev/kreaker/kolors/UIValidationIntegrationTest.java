...
class UIValidationIntegrationTest {
   ...
   class PageStructureValidationTests {
      ...
      void shouldRenderMainPageWithProperHtmlStructure() throws Exception {
         ...
         MvcResult result = mockMvc.perform(get("/combinations/")).andExpect(status().isOk())
      ...
      void shouldRenderEditPageWithProperFormStructure() throws Exception {
         ...
         MvcResult result = mockMvc.perform(get("/combinations/" + combination.getId() + "/edit"))
                  .andExpect(status().isOk()).andExpect(view().name("combinations/edit"))
                  .andReturn();
         ...
         assertThat(content)
                  .contains("action=\"/combinations/" + combination.getId() + "/update\"");
      ...
      void shouldRenderViewPageWithProperColorDisplay() throws Exception {
         ...
         MvcResult result = mockMvc.perform(get("/combinations/" + combination.getId()))
                  .andExpect(status().isOk()).andExpect(view().name("combinations/view"))
                  .andReturn();
   ...
   class ResponsiveDesignValidationTests {
      ...
      void shouldHandleDifferentScreenSizeSimulations() throws Exception {
         ...
         String[] userAgents = {"Mozilla/5.0 (iPhone; CPU iPhone OS 14_0 like Mac OS X)", // Mobile
         ...
         for (String userAgent : userAgents) {
            MvcResult result =
                     mockMvc.perform(get("/combinations/").header("User-Agent", userAgent))
   ...
   class FormValidationTests {
      ...
      void shouldDisplayValidationErrorsProperly() throws Exception {
         ...
         MvcResult result = mockMvc.perform(post("/combinations/create").param("name", "ab") // Too
                                                                                             // short
                  .param("colorCount", "1").param("colors[0].hexValue", "INVALID") // Invalid hex
                  .param("colors[0].position", "1")).andExpect(status().isOk()).andReturn();
   ...
   class CrossBrowserCompatibilityTests {
      ...
      void shouldHandleDifferentHttpAcceptHeaders() throws Exception {
         ...
         for (String acceptHeader : acceptHeaders) {
            mockMvc.perform(get("/combinations/").header("Accept", acceptHeader))
                     .andExpect(status().isOk())
                     .andExpect(content().contentTypeCompatibleWith(MediaType.TEXT_HTML));
         }
      }
   ...
   class DynamicContentValidationTests {
      ...
      void shouldHandleSearchResultsDisplayCorrectly() throws Exception {
         ...
         // Test search by name
         MvcResult nameResult = mockMvc.perform(get("/combinations/").param("search", "Ocean"))
                  .andExpect(status().isOk()).andReturn();
         ...
         MvcResult countResult = mockMvc.perform(get("/combinations/").param("colorCount", "2"))
                  .andExpect(status().isOk()).andReturn();
         ...
         MvcResult hexResult = mockMvc.perform(get("/combinations/").param("hexValue", "0077BE"))
                  .andExpect(status().isOk()).andReturn();
      ...
      void shouldDisplayEmptyStateCorrectly() throws Exception {
         ...
         assertThat(content).containsAnyOf("No hay combinaciones guardadas",
                  "Crea tu primera combinación", "No combinations found", "No color combinations",
                  "Create your first", "empty");
