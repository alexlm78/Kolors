...
public interface ColorCombinationRepository extends JpaRepository<ColorCombination, Long> {
    ...
    /**
     * Searches the most popular combinations (for now only the most recent) In
     * the future a popularity or usage field could be added
     */
     ...
     * Searches combinations by name with pagination
     */
    Page<ColorCombination> findByNameContainingIgnoreCaseOrderByNameAsc(String name, Pageable pageable);

    /**
     * Searches combinations by color count range with pagination
     */
    Page<ColorCombination> findByColorCountBetweenOrderByCreatedAtDesc(Integer minColors, Integer maxColors, Pageable pageable);
