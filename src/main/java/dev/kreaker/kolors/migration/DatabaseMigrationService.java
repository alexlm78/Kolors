...
public class DatabaseMigrationService {
        ...
        try {
            ...
            for (ColorCombination combination : allCombinations) {
                try {
                    ...
                    invalidCombinations++;
                    String error =
                            "Failed to validate combination ID "
                                    + combination.getId()
                                    + ": "
                                    + e.getMessage();
            ...
            if (invalidCombinations == 0) {
                ...
                logger.info(
                        "Data validation completed successfully. Validated {} combinations",
                        validCombinations);
            } else {
    ...
    private boolean validateSingleCombination(
            ColorCombination combination, MigrationResult result) {
        try {
            ...
            logger.error(
                    "Error validating combination ID {}: {}",
                    combination.getId(),
                    e.getMessage(),
                    e);
            return false;
    ...
    private boolean validateCombinationColors(
            ColorCombination combination, MigrationResult result) {
        ...
        for (int i = 0; i < colors.size(); i++) {
            ...
            if (color.getHexValue() == null
                    || !HEX_PATTERN.matcher(color.getHexValue()).matches()) {
                result.addError(
                        "Combination ID "
                                + combination.getId()
