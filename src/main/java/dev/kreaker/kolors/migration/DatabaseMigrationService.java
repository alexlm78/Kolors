...
public class DatabaseMigrationService {
      ...
      try {
         ...
         for (ColorCombination combination : allCombinations) {
            try {
               ...
               invalidCombinations++;
               String error = "Failed to validate combination ID " + combination.getId() + ": "
                        + e.getMessage();
         ...
         if (invalidCombinations == 0) {
            ...
            logger.info("Data validation completed successfully. Validated {} combinations",
                     validCombinations);
         } else {
            result.setSuccess(false);
            logger.warn("Data validation found issues. Valid: {}, Invalid: {}", validCombinations,
                     invalidCombinations);
         }
   ...
   private boolean validateSingleCombination(ColorCombination combination, MigrationResult result) {
      try {
         ...
         logger.error("Error validating combination ID {}: {}", combination.getId(), e.getMessage(),
                  e);
         return false;
      }
   }
   ...
   private boolean isValidCombination(ColorCombination combination, MigrationResult result) {
      if (combination.getName() == null || combination.getName().trim().isEmpty()) {
         result.addError(
                  "Invalid combination ID " + combination.getId() + ": name is null or empty");
         return false;
      }

      if (combination.getName().length() < 3) {
         result.addError("Invalid combination ID " + combination.getId()
                  + ": name too short (< 3 characters)");
         return false;
      }

      if (combination.getColorCount() == null || combination.getColorCount() < 1) {
         result.addError("Invalid combination ID " + combination.getId() + ": invalid color count "
      ...
      if (combination.getCreatedAt() == null) {
         result.addError(
                  "Invalid combination ID " + combination.getId() + ": missing creation date");
         return false;
      }
   ...
   private boolean validateCombinationColors(ColorCombination combination, MigrationResult result) {
      ...
      if (colors.size() != combination.getColorCount()) {
         result.addError(
                  "Combination ID " + combination.getId() + " color count mismatch: expected "
      ...
      for (int i = 0; i < colors.size(); i++) {
         ...
         if (color.getHexValue() == null || !HEX_PATTERN.matcher(color.getHexValue()).matches()) {
            result.addError(
                     "Combination ID " + combination.getId() + " has invalid hex value at position "
         ...
         if (color.getPosition() == null || color.getPosition() != (i + 1)) {
            result.addError(
                     "Combination ID " + combination.getId() + " has invalid position at index " + i
                              + ": expected " + (i + 1) + ", found " + color.getPosition());
            return false;
         }
      }
   ...
   /**
    * Checks if there is legacy data available for migration. Always returns false as legacy data
   ...
   public MigrationStatistics getMigrationStatistics() {
      ...
      try {
         ...
         stats.setSingleColorCombinationCount(
                  colorCombinationRepository.findByColorCount(1).size());
