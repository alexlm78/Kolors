...
/**
 * Controller for managing post-migration data validation and cleanup. Legacy migration has been
 * completed and legacy code removed. Provides administrative interface for data integrity
 * validation.
 */
@Controller
    ...
    try {
      ...
      if (!backupCreated) {
        redirectAttributes.addFlashAttribute(
            "warning", "Failed to create backup, but proceeding with validation");
      }
      ...
      if (result.isSuccess()) {
        redirectAttributes.addFlashAttribute(
            "success", "Data validation completed successfully: " + result.getSummary());
      } else {
        redirectAttributes.addFlashAttribute(
            "error", "Data validation found issues: " + result.getSummary());
      }
  ...
  public ResponseEntity<String> resetMigrationStatus() {
    try {
      ...
      return ResponseEntity.internalServerError()
          .body("Error resetting migration status: " + e.getMessage());
    }
  }
