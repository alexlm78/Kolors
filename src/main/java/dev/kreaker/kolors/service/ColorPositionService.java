...
/**
 * Service for managing color positions in combinations Handles dynamic position
...
public class ColorPositionService {
    ...
    public void reorderPositionsAfterRemoval(Long combinationId, Integer removedPosition) {
        ...
        List<ColorInCombination> colorsToReorder = colorInCombinationRepository
                .findByCombinationIdAndPositionGreaterThanOrderByPosition(combinationId, removedPosition);
        ...
        for (ColorInCombination color : colorsToReorder) {
            ...
            logger.debug("Updated color position from {} to {} for color ID: {}",
                    oldPosition, color.getPosition(), color.getId());
        }
    ...
    public boolean validateSequentialPositions(List<ColorInCombination> colors) {
        ...
        for (int i = 0; i < colors.size(); i++) {
            if (colors.get(i).getPosition() != i + 1) {
                logger.warn("Non-sequential position found: expected {}, got {}",
                        i + 1, colors.get(i).getPosition());
                return false;
            }
        }
    ...
    public void fixSequentialPositions(Long combinationId) {
        ...
        for (int i = 0; i < colors.size(); i++) {
            ...
            if (!color.getPosition().equals(expectedPosition)) {
                logger.debug("Fixing position for color ID {}: {} -> {}",
                        color.getId(), color.getPosition(), expectedPosition);
                color.setPosition(expectedPosition);
                colorInCombinationRepository.save(color);
            }
        }

        logger.info("Fixed positions for {} colors in combination ID: {}", colors.size(), combinationId);
    }
    ...
    public boolean isPositionAvailable(Long combinationId, Integer position) {
        ...
        return !colorInCombinationRepository.existsByCombinationIdAndPosition(combinationId, position);
    }
    ...
    public List<Integer> getUsedPositions(Long combinationId) {
        ...
        return colorInCombinationRepository.findByCombinationIdOrderByPosition(combinationId)
                .stream()
