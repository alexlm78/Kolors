...
public class PerformanceMonitoringService {
   ...
   public void recordDatabaseOperation(String operationName, Duration executionTime) {
      try {
         ...
         if (executionTimeMs > SLOW_QUERY_THRESHOLD_MS) {
            ...
            performanceLogger.warn("SLOW QUERY detected: {} took {}ms", operationName,
                     executionTimeMs);
         } else if (executionTimeMs > WARNING_QUERY_THRESHOLD_MS) {
            performanceLogger.info("Query performance warning: {} took {}ms", operationName,
                     executionTimeMs);
         }
   ...
   public void recordServiceMethod(String methodName, Duration executionTime) {
      ...
      if (executionTimeMs > WARNING_QUERY_THRESHOLD_MS) {
         performanceLogger.info("Service method performance: {} took {}ms", methodName,
                  executionTimeMs);
      }

      metrics.compute("service." + methodName, (key, existing) -> {
         if (existing == null) {
   ...
   public DatabasePerformanceSummary getDatabasePerformanceSummary() {
      return new DatabasePerformanceSummary(totalQueries.get(), slowQueries.get(),
               calculateAverageQueryTime(), getSlowQueryPercentage());
   }
   ...
   public void logPerformanceSummary() {
      ...
      performanceLogger.info("Slow queries: {} ({}%)", summary.getSlowQueries(),
               summary.getSlowQueryPercentage());
      performanceLogger.info("Average query time: {}ms", summary.getAverageQueryTimeMs());

      // Log top 5 slowest operations
      metrics.entrySet().stream().sorted((e1, e2) -> Double
               .compare(e2.getValue().getAverageTimeMs(), e1.getValue().getAverageTimeMs()))
               .limit(5).forEach(entry -> {
                  PerformanceMetric metric = entry.getValue();
                  performanceLogger.info("Operation '{}': avg={}ms, max={}ms, count={}",
                           entry.getKey(), String.format("%.2f", metric.getAverageTimeMs()),
                           metric.getMaxTimeMs(), metric.getExecutionCount());
               });
   ...
   private double calculateAverageQueryTime() {
      return metrics.values().stream()
               ...
               .mapToDouble(PerformanceMetric::getAverageTimeMs).average().orElse(0.0);
   }
   ...
   public static class DatabasePerformanceSummary {
      ...
      public DatabasePerformanceSummary(long totalQueries, long slowQueries,
               double averageQueryTimeMs, double slowQueryPercentage) {
